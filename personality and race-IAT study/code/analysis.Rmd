...
ggplot(data_processed_after_exclusions, aes(x = D)) +
  ...
  scale_x_continuous(breaks = pretty_breaks(n = 6), limits = c(-1.1, 0.7)) +
...
For the X axis, I chose values from 1 to 6 because these are the boundaries and we represented the data from 1 to 6. I chose the values -1 to 0.75 for the Y axis because a larger bandwidth would result in too much white space and it would not be necessary because all data can be successfully mapped in this way. In addition, I added a zero line for the Y axis to emphasize the asymmetric distribution of the data.
...
plot_agree <- ggplot(mean_agree, aes(x = mean_agreeableness, y = D)) +
  ...
  scale_y_continuous(breaks = pretty_breaks(n = 5),  limits = c(-1, 0.75)) +
...
plot_open <- ggplot(mean_open, aes(x = mean_openness, y = D)) +
  ...
  scale_x_continuous(breaks = pretty_breaks(n = 6), limits = c(1, 6.2)) +
  scale_y_continuous(breaks = pretty_breaks(n = 6),  limits = c(-1, 0.75)) +
  expand_limits(x = c(1, 6), y = c(-1, 1)) +
  coord_cartesian(expand = FALSE) +
  theme_classic() +
  theme(title = element_text(face = "bold"),
